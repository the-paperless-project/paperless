...
from .models import Document, FileInfo, Tag, Scan
...
class Consumer:
    ...
    def consume_new_files(self):
        ...
        for scan in Scan.objects.filter(processed__isnull=True):
            self.try_consume_file(scan)
            scan.processed = timezone.now()
            scan.save()
    

    @transaction.atomic
    def try_consume_file(self, scan):
        """
    ...
    def _store(self, text, doc, thumbnail, date, scan):
